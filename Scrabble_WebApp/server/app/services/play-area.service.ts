...
export class PlayAreaService {
    ...
    private randomActionExpertVP(game: GameServer, player: Player) {
        if (!this.expertVPService.generateMoves(game, player)) {
    ...
    private randomActionVP(game: GameServer, player: Player) {
        ...
        if (probaMove < tenPercent) {
            ...
            this.virtualPService.generateMoves(game, player);
        } else {
            this.chatService.passCommand('!passer', game, player);
        }
    }
