...
export class ChatService {
    constructor(private validator: ValidationService, private endGameService: EndGameService) {}
    ...
    sendMessage(input: string, game: GameServer, player: Player): boolean {
        ...
        if (input[0] === '!') {
            ...
            if (
                (command === Commands.Place || command === Commands.Exchange || command === Commands.Pass) &&
                !(player.idPlayer === game.currentPlayerId)
            ) {
            ...
            // veritfy that the command is valid
    ...
    placeCommand(input: string, game: GameServer, player: Player) {
        ...
        if (this.validator.reserveIsEmpty(game.letterBank)) {
            if (this.validator.standEmpty(player)) {
