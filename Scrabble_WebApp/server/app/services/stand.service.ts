...
export class StandService {
    ...
    private initStandArray(lettreInit: string, letterBank: Map<string, LetterData>, player: Player) {
        const nbOccupiedSquare: number = lettreInit.length;

        for (
            let i = 0, j = GlobalConstants.SIZE_OUTER_BORDER_STAND;
            i < GlobalConstants.NUMBER_SLOT_STAND;
            i++, j += GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS
        ) {
            ...
            if (i < nbOccupiedSquare) {
                newLetter.weight = this.letterBankService.checkLetterWeight(lettreInit[i], letterBank);
                newLetter.value = lettreInit[i];

                newTile.letter = newLetter;
                newTile.bonus = '0';

                player.stand.push(newTile);
                this.writeLetterInStandMap(lettreInit[i], player);
            }
            // Fills the rest
            else {
                newLetter.weight = 0;
