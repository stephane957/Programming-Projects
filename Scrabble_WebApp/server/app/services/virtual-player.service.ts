...
export class VirtualPlayerService {
    ...
    getTrie() {
        this.trie = this.dictService.getTrie();
    }
    ...
    generateMoves(game: GameServer, player: Player) {
        ...
        for (const dir of directions) {
            ...
            this.computeCrossChecks(game.board);
            ...
            for (const anchorPos of this.anchorSquares) {
                if (
                    ...
                    const currentNode = this.trie.getNode(partialWord);

                    if (currentNode !== undefined && currentNode) {
                        this.extendRight(partialWord, currentNode, anchorPos, false, game, letterStand);
                    }
                } else {
                    this.leftPart('', this.trie.root, this.getLimit(anchorPos, game.board), anchorPos, game, letterStand);
                }
            }
        }
    ...
    randomPlacement(game: GameServer, player: Player) {
        ...
        this.moves[1] = [];
        this.moves[2] = [];
        // si le moveToPlay n'esxiste pas, le joueur virtuel passe son tour
    ...
    private computeCrossChecks(board: Tile[][]) {
        this.validEntries = new Map<Vec2, Set<string>>();
        for (let i = 1; i < board.length - 1; i++) {
            for (let j = 1; j < board[i].length - 1; j++) {
                const pos: Vec2 = { x: i, y: j };
                if (board[i][j].letter.value === '') {
                    ...
                    if (beforeLetters.length === 0 && afterLetters.length === 0) {
                        ...
                        for (const char of alphabet) {
                            ...
                            if (this.dictService.containsWord(word)) {
                                set.add(char);
                            }
                        }
                    }
