...
export class ValidationService {
    ...
    entryIsTooLong(input: string): boolean {
        // verify that the entry of the user isnt longer thant 512 caracters
    ...
    verifyPlacementOnBoard(splittedInput: string[], game: GameServer, player: Player): string {
        ...
        if (game.noTileOnBoard && !this.lettersTouchH8Sqare(word, position)) {
    ...
    reserveIsEmpty(letterBank: Map<string, LetterData>): boolean {
        if (this.letterBankService.getNbLettersInLetterBank(letterBank) === 0) {
            return true;
        }
        return false;
    }
    ...
    private isPlaceInputValid(splittedInput: string[], game: GameServer): string {
        ...
        // if the command doesnt have any arguments or a valid position we return false
    ...
    private areLettersUsedFromCanvasWellPlaced(word: string, position: string, game: GameServer, player: Player) {
        const lettreWay: string = position.slice(GlobalConstants.POSITION_LAST_LETTER);
    ...
    private wordFitsBoard(word: string, position: string): boolean {
        ...
        if (lettreWay === 'h') {
            if (indexColumn + Number(word.length) > GlobalConstants.NUMBER_SQARE_H_AND_W) return false;
        } else {
            if (indexLine + Number(word.length) > GlobalConstants.NUMBER_SQARE_H_AND_W) return false;
        }
        return true;
    }

    private isPlacePositionValid(position: string): boolean {
        const lenghtDoubleCharColumnInput = 4;
        // concatenation of the conditions of validy of a boolean entry
        if (position.length === 3) {
            return this.isLineValid(position[0]) && this.isSingleDigitColumnValid(position[1]) && this.isOrientationValid(position[2]);
        } else if (position.length === lenghtDoubleCharColumnInput) {
    ...
    private lettersTouchH8Sqare(word: string, position: string): boolean {
    ...
    private lettersHaveContactWithOthers(word: string, position: string, game: GameServer): boolean {
        ...
        for (let i = 0; i < word.length; i++) {
            if (
                game.board[indexLine][indexColumn].letter.value !== '' ||
                game.board[indexLine + 1][indexColumn].letter.value !== '' ||
                game.board[indexLine - 1][indexColumn].letter.value !== '' ||
                game.board[indexLine][indexColumn + 1].letter.value !== '' ||
                game.board[indexLine][indexColumn - 1].letter.value !== ''
            ) {
                return true;
            }
    ...
    private lettersAreOnBoardOrStand(word: string, game: GameServer, player: Player): boolean {
        for (const lettre of word) {
            ...
            let lettreToCheck: string = lettre;

            // condition that veify that its not a blank letter
            if (lettre === lettre.toUpperCase()) {
                lettreToCheck = '*';
            }

            const doesMapStandHaveLetter = player.mapLetterOnStand.has(lettreToCheck);
            const doesMapBoardHaveLetter = game.mapLetterOnBoard.has(lettreToCheck);
            const nbRepetitionLetter = this.charCount(word, lettreToCheck);

            switch (true) {
                case doesMapStandHaveLetter && doesMapBoardHaveLetter: {
                    if (player.mapLetterOnStand.get(lettreToCheck).value + game.mapLetterOnBoard.get(lettreToCheck).value < nbRepetitionLetter) {
                        return false;
                    }
                    break;
                }
                case doesMapStandHaveLetter && !doesMapBoardHaveLetter: {
                    if (player.mapLetterOnStand.get(lettreToCheck).value < nbRepetitionLetter) {
                        return false;
                    }
                    break;
                }
                case !doesMapStandHaveLetter && doesMapBoardHaveLetter: {
                    if (game.mapLetterOnBoard.get(lettreToCheck).value < nbRepetitionLetter) {
                        return false;
                    }
                    break;
                }
    ...
    private belongsInAlphabet(letter: string): boolean {
        for (const caracter of ALPHABET) {
            if (caracter === letter.toLowerCase()) {
                return true;
            }
        }
        return false;
    ...
    // if the ligne is a number it must be between 1 and 15
