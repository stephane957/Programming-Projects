...
export class DrawingBoardService {
    ...
    drawBoardInit(bonusBoard: string[][]) {
        // we take out the first line and column because she isnt used for the drawing of the board
        ...
        for (let x = 0; x < GlobalConstants.NUMBER_SQARE_H_AND_W; x++) {
            for (let y = 0; y < GlobalConstants.NUMBER_SQARE_H_AND_W; y++) {
        ...
        const ascciCodeStartLetters = 64;
        const fontSizeLettersOnSide = 25;
        const borderTopAndLeftBig = 14;
        const borderTopAndLeftLittle = 5;
        // The variable widthEachSqare being not a round number there is a rest that we need to use
        // in the next function
        const roundedRest = 1;
        for (
            let i = GlobalConstants.SIZE_OUTER_BORDER_BOARD + GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS / 2, j = 1;
            i < GlobalConstants.WIDTH_BOARD_NOBORDER + roundedRest;
            i += GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS, j++
        ) {
            ...
            if (j.toString().length === 1) {
                this.boardCanvas.fillText(
                    j.toString(),
                    i - GlobalConstants.WIDTH_EACH_SQARE - GlobalConstants.WIDTH_LINE_BLOCKS / 2 + borderTopAndLeftBig,
                    jumpOfATile,
                );
            } else {
                this.boardCanvas.fillText(
                    j.toString(),
                    i - GlobalConstants.WIDTH_EACH_SQARE - GlobalConstants.WIDTH_LINE_BLOCKS / 2 + borderTopAndLeftLittle,
                    jumpOfATile,
                );
            }
            this.boardCanvas.fillText(
                String.fromCharCode(ascciCodeStartLetters + j),
                borderTopAndLeftBig,
                i - GlobalConstants.WIDTH_EACH_SQARE - GlobalConstants.WIDTH_LINE_BLOCKS / 2 + jumpOfATile,
            );

            // Since our for loop stops at the index 14 we have to implement it manually
            // for the 15th number
            if (j === GlobalConstants.NUMBER_SQARE_H_AND_W - 1) {
                j++;
                this.boardCanvas.fillText(j.toString(), i + borderTopAndLeftLittle, jumpOfATile);
                this.boardCanvas.fillText(String.fromCharCode(ascciCodeStartLetters + j), borderTopAndLeftBig, i + jumpOfATile);
            }
        }
    ...
    reDrawBoard(bonusBoard: string[][], board: Tile[][], letterBank: Map<string, LetterData>) {
        ...
        for (let x = 0; x < GlobalConstants.NUMBER_SQARE_H_AND_W + 2; x++) {
            for (let y = 0; y < GlobalConstants.NUMBER_SQARE_H_AND_W + 2; y++) {
    ...
    drawHorizontalArrowDirection(verticalPosTile: number, horizontalPosTile: number) {
        ...
        this.boardCanvas.moveTo(startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTile, startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / 2);
        this.boardCanvas.lineTo(
            startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTileInDecimal,
            startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / 2,
        );
        this.boardCanvas.stroke();
        this.boardCanvas.lineTo(
            startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / 2,
            startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTileInDecimal,
        );
        this.boardCanvas.stroke();
        this.boardCanvas.lineTo(
            startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTileInDecimal,
        ...
        this.boardCanvas.stroke();
        this.boardCanvas.lineTo(startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / 2, startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTile);
        this.boardCanvas.stroke();
    ...
    drawVerticalArrowDirection(verticalPosTile: number, horizontalPosTile: number) {
        ...
        const startPosXPx = this.startingPosPxOfTile(verticalPosTile - 1);
        const startPosYPx = this.startingPosPxOfTile(horizontalPosTile - 1);
        this.boardCanvas.moveTo(startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / 2, startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTile);
        this.boardCanvas.lineTo(
            startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / 2,
            startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTileInDecimal,
        );
        ...
        this.boardCanvas.stroke();
        this.boardCanvas.lineTo(startPosXPx + GlobalConstants.WIDTH_EACH_SQARE / oneFifthOfTile, startPosYPx + GlobalConstants.WIDTH_EACH_SQARE / 2);
        this.boardCanvas.stroke();
        this.arrowPosX = verticalPosTile;
        this.arrowPosY = horizontalPosTile;
        this.boardCanvas.lineWidth = GlobalConstants.WIDTH_LINE_BLOCKS;
    ...
    drawTileAtPos(xPos: number, bonusBoard: string[][], yPos: number, width?: number) {
        if (xPos > GlobalConstants.NUMBER_SQARE_H_AND_W || yPos > GlobalConstants.NUMBER_SQARE_H_AND_W) {
            return;
        }
        const savedFont = this.boardCanvas.font;
        const fontSizeBonusWord = 'bold 11px system-ui';
        ...
        this.boardCanvas.font = fontSizeBonusWord;
        const xPosPx =
            xPos * (GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS) +
            GlobalConstants.SIZE_OUTER_BORDER_BOARD -
            marginForRoundedNumberAndLook / 2;
        const yPosPx =
            yPos * (GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS) +
            GlobalConstants.SIZE_OUTER_BORDER_BOARD -
            marginForRoundedNumberAndLook / 2;
        if (width || width === 0) {
            yPos += 1;
        ...
        if (
            xPos === Math.floor(GlobalConstants.NUMBER_SQARE_H_AND_W / 2) &&
            yPos - 1 === Math.floor(GlobalConstants.NUMBER_SQARE_H_AND_W / 2) &&
            this.isArrowPlaced
        ) {
        ...
        if (width || width === 0) {
            ...
            this.boardCanvas.fillRect(xPosPx + width, yPosPx + width, GlobalConstants.WIDTH_EACH_SQARE, GlobalConstants.WIDTH_EACH_SQARE);
        } else {
            this.boardCanvas.fillRect(
                xPosPx,
                yPosPx,
                GlobalConstants.WIDTH_EACH_SQARE + marginForRoundedNumberAndLook,
                GlobalConstants.WIDTH_EACH_SQARE + marginForRoundedNumberAndLook,
            );
        }
        if (bonusBoard[xPos][yPos] !== 'xx') {
            ...
            if (xPos === (GlobalConstants.NUMBER_SQARE_H_AND_W - 1) / 2 && yPos === (GlobalConstants.NUMBER_SQARE_H_AND_W - 1) / 2) {
                this.boardCanvas.font = savedFont;
                return;
            }
            if (bonusBoard[xPos][yPos].includes('letter')) {
                this.boardCanvas.fillText(
                    'LETTRE',
                    xPosPx +
                        (GlobalConstants.WIDTH_EACH_SQARE - this.boardCanvas.measureText('LETTRE').width) / 2 +
                        marginForRoundedNumberAndLook / 2,
                    yPosPx + GlobalConstants.WIDTH_EACH_SQARE / 2 + marginForRoundedNumberAndLook / 2,
                );
            } else {
                this.boardCanvas.fillText(
                    'MOT',
                    xPosPx + (GlobalConstants.WIDTH_EACH_SQARE - this.boardCanvas.measureText('MOT').width) / 2 + marginForRoundedNumberAndLook / 2,
                    yPosPx + GlobalConstants.WIDTH_EACH_SQARE / 2 + marginForRoundedNumberAndLook / 2,
                );
            }
            if (bonusBoard[xPos][yPos].includes('x2')) {
                this.boardCanvas.fillText(
                    'x2',
                    xPosPx + (GlobalConstants.WIDTH_EACH_SQARE - this.boardCanvas.measureText('x2').width) / 2 + marginForRoundedNumberAndLook / 2,
                    yPosPx + GlobalConstants.WIDTH_EACH_SQARE / 2 + borderTopAndLeft,
                );
            } else {
                this.boardCanvas.fillText(
                    'x3',
                    xPosPx + (GlobalConstants.WIDTH_EACH_SQARE - this.boardCanvas.measureText('x3').width) / 2 + marginForRoundedNumberAndLook / 2,
                    yPosPx + GlobalConstants.WIDTH_EACH_SQARE / 2 + borderTopAndLeft,
                );
            }
        }
    ...
    findTiletoPlaceArrow(positionPx: Vec2, board: Tile[][], bonusBoard: string[][]) {
        ...
        const verticalPosTile: number =
            Math.floor((1 / (GlobalConstants.WIDTH_BOARD_NOBORDER / positionPx.x)) * GlobalConstants.NUMBER_SQARE_H_AND_W) + 1;
        const horizontalPosTile: number =
            Math.floor((1 / (GlobalConstants.WIDTH_BOARD_NOBORDER / positionPx.y)) * GlobalConstants.NUMBER_SQARE_H_AND_W) + 1;
        if (board[horizontalPosTile][verticalPosTile].old) {
            // check if the tile has a letter
            return; // if it does then we dont draw an arrow
        }
        if ((this.arrowPosX <= GlobalConstants.NUMBER_SQARE_H_AND_W, this.arrowPosY <= GlobalConstants.NUMBER_SQARE_H_AND_W)) {
    ...
    private startingPosPxOfTile(tilePos: number): number {
        return (tilePos + 1) * GlobalConstants.WIDTH_EACH_SQARE + tilePos * GlobalConstants.WIDTH_LINE_BLOCKS;
    }
    ...
    private redrawStar(xPosPx: number, yPosPx: number, width?: number) {
        if (width) {
            this.boardCanvas.fillRect(
                xPosPx + width,
                yPosPx + width,
                GlobalConstants.WIDTH_EACH_SQARE - width,
                GlobalConstants.WIDTH_EACH_SQARE - width,
            );
        } else {
            return;
    ...
    private drawStar(centerX: number, centerY: number) {
        ...
        const radius = GlobalConstants.WIDTH_EACH_SQARE / 2 - shiftValueForCenteredStar;
