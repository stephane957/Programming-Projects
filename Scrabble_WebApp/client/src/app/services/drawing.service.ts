...
export class DrawingService {
    ...
    areLettersLeftClicked(stand: Tile[]) {
        for (const tile of stand) {
            if (tile.color === '#ff6600') {
                return true;
            }
        }
        return false;
    ...
    private initStand() {
        ...
        for (
            let i = GlobalConstants.SIZE_OUTER_BORDER_STAND + GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS / 2;
            i < GlobalConstants.DEFAULT_WIDTH_STAND;
            i += GlobalConstants.WIDTH_EACH_SQARE + GlobalConstants.WIDTH_LINE_BLOCKS
        ) {
